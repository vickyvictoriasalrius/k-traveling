    ...
    return (
        <div className='text-[10px] lg:text-[11px] xl:text-[13px] font-poppins'>
            <div className={`${currentPath.startsWith('/dashboard') ? 'px-10' : 'lg:px-36 sm:px-10 px-5'} ${showProfile || showModal || currentPath.startsWith('/dashboard') ? 'bg-opacity-100' : 'bg-opacity-[85%] backdrop-blur-lg dark:bg-opacity-[85%] dark:backdrop-blur-lg'} z-[51] fixed flex items-center justify-between w-full h-12 md:h-14 xl:h-16 bg-white dark:bg-primaryblack shadow-navbar3 md:shadow-navbar2 xl:shadow-navbar dark:shadow-slate-600 text-primaryblack`}>
                <button onClick={() => currentPath.startsWith("/dashboard") ? router.push("/dashboard/users") : router.push(linkList.Home)} className='cursor-default cursor-scale lg:cursor-none'><Image className='w-auto h-7 md:h-8 xl:h-9' src={isDark ? "/images/Logo-dark.png" : "/images/Logo.png"} width={500} height={500} /></button>
                ...
                <div className="items-center hidden gap-10 font-medium lg:gap-14 xl:gap-16 sm:flex">
                    {keys.map((key, index) => (
                        <div key={index}>
                            <button
                                ...
                                className={`cursor-scale cursor-default lg:cursor-none dark:text-white hover:text-primaryred dark:hover:text-primaryyellow ${currentPath === modifiedLinkList[key] && currentPath === "/login_register" ? 'text-white bg-primaryred dark:bg-primaryyellow'
                    ...
                    {user && (
                        <div className='relative flex w-full'>
                            <button onClick={handleDropDownToggle} className='flex items-center cursor-default w-fit cursor-scale lg:cursor-none'>
                            ...
                            <div className={`absolute right-0 w-32 xl:w-36 z-10 mt-14 bg-white dark:bg-primaryblack dark:text-slate-200 shadow-dropdown dark:shadow-slate-600 text-primaryblack rounded-lg ${dropDownHidden ? 'hidden' : ''}`}>
                                <div className="px-2">
                                    <button
                                        onClick={handleShowProfile} className='flex items-center justify-between w-full px-2 py-1 my-2 rounded-md cursor-default cursor-scale lg:cursor-none xl:px-4 hover:bg-slate-200 dark:hover:bg-slate-700'>
                                        <h1>Profile</h1>
                                    ...
                                    <button
                                        onClick={currentPath.startsWith("/dashboard") ? () => router.push("/") : () => router.push("dashboard/users")} className={`${user.role === "admin" ? '' : 'hidden'} cursor-scale cursor-default lg:cursor-none flex items-center w-full justify-between px-2 xl:px-4 py-1 my-2 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700`}>
                                    ...
                                    <button
                                        onClick={handleTheme} className='flex items-center justify-between w-full px-2 py-1 my-2 rounded-md cursor-default cursor-scale lg:cursor-none xl:px-4 hover:bg-slate-200 dark:hover:bg-slate-700'>
                                    ...
                                    <button
                                        onClick={logout} className='flex items-center justify-between w-full px-2 py-1 my-2 rounded-md cursor-default cursor-scale lg:cursor-none xl:px-4 hover:bg-slate-200 dark:hover:bg-slate-700'>
                                        <h1>Logout</h1>
                ...
                <div className='flex flex-col gap-5'>
                    {keys.map((key, index) => (
                        <div key={index}>
                            <button
                                onClick={() => router.push(modifiedLinkList[key])}
                                className={`cursor-scale cursor-default lg:cursor-none dark:text-white hover:text-primaryred dark:hover:text-primaryyellow ${currentPath === modifiedLinkList[key] && currentPath === "/loginregis" ? 'text-white bg-primaryred dark:bg-primaryyellow'
                                    : (currentPath === modifiedLinkList[key] ? 'text-primaryred dark:text-yellowprimary' : '')} ${key === "Login" ? "px-3 lg:px-4 py-[6px] lg:py-2 border hover:text-white dark:hover:text-white hover:bg-primaryred dark:hover:bg-primaryyellow rounded-lg border-primaryred dark:border-primaryyellow" : ""}`}>
                                {key}
                            </button>
                        </div>
